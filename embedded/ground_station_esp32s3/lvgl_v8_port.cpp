...
// VSYNC 回调运行在中断上下文，使用原子操作避免与界面任务读取计数时竞争。
static uint32_t lvgl_port_vsync_count = 0;
...
{
    __atomic_add_fetch(&lvgl_port_vsync_count, 1, __ATOMIC_RELAXED);
...
{
    return __atomic_load_n(&lvgl_port_vsync_count, __ATOMIC_RELAXED);
}
